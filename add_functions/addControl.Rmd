...
icon the icon(s) for markers; an icon is represented by an R list of the form list(iconUrl = "?", iconSize = and you can use icons() to create multiple icons; note when you use an R list
...
smoothFactor how much to simplify the polyline on each zoom level (more means better performance
and less accurate representation)
